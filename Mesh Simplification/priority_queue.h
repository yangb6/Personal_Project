...
void PriorityQueue<T>::remove(T element) {
  ...
  //
    ...
    if(itr ==locations.end())
    {
        std::cout<<"ERROR! Couldn't find the elements: "<<element<<std::endl;
    }
    assert(element == itr->first);
    int index = itr->second;
    int success = locations.erase(m_heap[index]);
    assert(success == 1);
    m_heap[index] = m_heap.back();
    m_heap.pop_back();
    locations[m_heap[index]] = index;
    update_position(m_heap[index]);
}
...
void PriorityQueue<T>::percolate_up(int i) {
  ...
  //
    ...
    int currentIndex = i;
    while(currentIndex>0&&currentIndex<(m_heap.size()-1))
    {
        int parent_index = get_parent(currentIndex);
        if(m_heap[currentIndex]->getPriorityValue() <
           m_heap[parent_index]->getPriorityValue())
        {
            T temp = m_heap[parent_index];
            m_heap[parent_index] = m_heap[currentIndex];
            m_heap[currentIndex] = temp;
            locations[m_heap[parent_index]] = parent_index;
            locations[m_heap[currentIndex]] = currentIndex;
...
void PriorityQueue<T>::percolate_down(int i) {
  ...
  //
    ...
    {
        child_index = get_left_child(i);
        if(has_right_child(i))
        {
            int right_child_index = get_right_child(i);
            if(m_heap[child_index]->getPriorityValue() >
               m_heap[right_child_index]->getPriorityValue())
            {
                child_index = right_child_index;
            }
        }
        if(m_heap[i]->getPriorityValue() > m_heap[child_index]->getPriorityValue())
        {
            std::swap(m_heap[i],m_heap[child_index]);
            locations[m_heap[child_index]] = child_index;
            locations[m_heap[i]] = i;
            i= child_index;
        }
        else
