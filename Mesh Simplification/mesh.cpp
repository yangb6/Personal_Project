...
bool Mesh::Collapse() {
  ...
  assert (a->getID() < b->getID());
    a->Average(b);
 //Find the related set from vertex b.
   std::set<Triangle*> triangles_b = b->getTriangles();
    //Delete the two triangle and reset related values.
  //
  // ASSIGNMENT: Finish the implementation of this function
  //
    //Remove the triangle and add the new triangle.
    std::set<Triangle*>::iterator itr = triangles_b.begin();
    while(itr != triangles_b.end())
    {
        Triangle *t = *itr;
        itr ++;
        if(t->HasVertex(a) && t->HasVertex(b))
        {
            ...
            if(t->getVertex(0) == b)
            {
                ...
                CollectEdgesWithVertex(t->getVertex(1),new_set);
                CollectEdgesWithVertex(t->getVertex(2),new_set);
            ...
            else if(t->getVertex(1) == b)
            {
                ...
                RemoveEdge(b, t->getVertex(2));
                CollectEdgesWithVertex(t->getVertex(0),new_set);
                CollectEdgesWithVertex(t->getVertex(2),new_set);
            }
            else
            {
            ...
            RemoveTriangle(t);
            ReCalculateEdges(new_set);
        }
        else
        {
            std::set<Edge*>  new_set;
            //We just need to reset the one vertex of the triangle and add it to the a vertex.
            if(t->getVertex(0) == b)
            {
                RemoveEdge(b, t->getVertex(1));
                RemoveEdge(b, t->getVertex(2));
                AddTriangle(a, t->getVertex(1),t->getVertex(2));
                CollectEdgesWithVertex(a,new_set);
                CollectEdgesWithVertex(t->getVertex(1),new_set);
                CollectEdgesWithVertex(t->getVertex(2),new_set);
